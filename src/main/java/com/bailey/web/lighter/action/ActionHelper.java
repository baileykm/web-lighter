...
import com.bailey.web.lighter.annotation.*;
import com.bailey.web.lighter.utils.ClassHelper;
import com.bailey.web.lighter.utils.GsonUTCDateAdapter;
import com.bailey.web.lighter.utils.GsonUtil;
import com.bailey.web.lighter.utils.file.*;
...
public class ActionHelper {
    ...
    public static void initRequestHandlers() {
        ...
        ActionLogger.logger.info("RequestHandler Initialized in " + (System.currentTimeMillis() - start) + "ms. " + requestHandlers.size() + " request(s) mapped.");
        if ( ActionLogger.logger.isTraceEnabled()) {
            ActionLogger.logger.trace(getRequestHandlersReport());
        }
    }
     ...
     * @return 映射表
     */
    private static String getRequestHandlersReport() {
        ...
        sb.append(line);
        sb.append(" Web-lighter Action Mapping Report ");
        sb.append(line);
    ...
    public void execute(RequestHandler handler, String url, HttpServletRequest req, HttpServletResponse resp) throws ActionException, IOException {
        ActionLogger.logger.trace(WebLighterConfig.getUrlPrefix() + url + " ==> " + handler.getActionClass().getName() + "." + handler.getMethod().getName() + "()");
        ...
        if (!method.isAnnotationPresent(Download.class) || actionResult.getCode() < 0) {
            ...
            if (!(actionResult.getResult() instanceof DownloadFileInfo)) {
                throw new ActionException("The result property of ActionResult REQUIRE an instance of DownloadFileInfo");
            }
     ...
     * @see ActionResult
     */
    private ActionResult invoke(RequestHandler handler,
                                HttpServletRequest req,
                                HttpServletResponse resp,
                                Map<String, JsonElement> requestParameters,
                                List<UploadFileInfo> fileInfos) throws ActionException {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (param.isAnnotationPresent(Inject.class)) {
                // 判断是否有默认构造函数, 若有则直接实例化之; 否则尝试使用getInstance()获得实例; 若均失败, 则抛出异常

                Class<?> injectClass = param.getType();
                try {
                    try {
                        paramValues[i] = injectClass.newInstance();     // 调用默认构造函数实例化
                    } catch (InstantiationException | IllegalAccessException e) {
                        paramValues[i] = injectClass.getDeclaredMethod("getInstance").invoke(null);   // 调用 getInstance() 方法, 获得实例
                    }
                } catch (Exception e) {
                    throw new ActionException("Failed to get instance for @Inject parameter: " + injectClass.getName(), e);
                }
                continue;
            }
            ...
            if (param.isAnnotationPresent(Param.class)) {                       // 上行参数
                ...
                if (paramValue == null || paramValue instanceof JsonNull) {     // null
                    paramValues[i] = null;
                    continue;
                }
                ...
                try {
                    ...
                    throw new ActionException("Parameter parsing error: " + paramName, e);
                }
            }
        }

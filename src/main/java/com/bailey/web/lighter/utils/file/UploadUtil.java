...
public class UploadUtil {
    ...
    public UploadResult upload(HttpServletRequest request, String uploadRelativePath, String serverNameRule, int maxFileSize, int maxRequestSize) {
        ...
        if (!uploadFolder.exists()) {
            if (!uploadFolder.mkdirs()) throw new RuntimeException("Make folder(s) for upload error: " + uploadPath);
        }
                ...
                for (FileItem item : formItems) {
                    ...
                    if (!item.isFormField()) {      // 文件
                        ...
                        result.addFile(fieldName, origFileName, item.getSize(), item.getContentType(), serverFileName,
                                uploadRelativePath + (uploadRelativePath.endsWith(File.separator)?"":File.separator) + serverFileName
                                , filePath);
                        ActionLogger.logger.info("File upload success: " + filePath);
                    } else {    // 普通表单字段

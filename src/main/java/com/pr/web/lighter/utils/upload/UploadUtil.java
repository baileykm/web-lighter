...
public class UploadUtil {
     ...
     * @param serverFileName 服务器端保存文件时使用的文件名, null 表示使用前端原文件名保存
     * @param maxFileSize 允许上传的单个文件最大字节数
     * @param maxRequestSize HttpServletRequest最大字节数
     * @return 文件上传处理结果
     */
    public UploadResult upload(HttpServletRequest request, String uploadRelativePath, String serverFileName, int maxFileSize, int maxRequestSize) {
            ...
            if (formItems != null && formItems.size() > 0) {
                ...
                for (FileItem item : formItems) {
                    ...
                    if (!item.isFormField()) {      // 文件
                        ...
                        if (serverFileName == null) serverFileName = origFileName;    // 传入的服务器端文件名为null, 则使用客户端原文件名保存
